...
import 'local_word_timing_repository.dart';
...
class _WordDisplay extends ConsumerWidget {
  ...
  static int _activeWord(
    ...
    // Show first word when duration is unknown rather than nothing.
    if (durMs <= 0) return 0;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Watch position only when THIS ayah is active.
    // select() returns null for every other ayah → no rebuild for non-playing rows.
    // For the active ayah, posMs updates every ~16 ms via audioProvider's
    // positionStream subscription, giving smooth per-frame highlighting.
    final posMs = ref.watch(audioProvider.select((s) {
      if (s.surahNumber != surah.id ||
    ...
    final isAudioActive = posMs != null;

    // Word timing priority: QDC API (12 reciters, highest accuracy) → bundled
    // alignment JSON (remaining reciters, generated by scripts/align_reciters.py).
    // Split all ?[...] subscripts onto their own lines: the old build_runner
    // analyzer (7.6.0 / Dart 3.9.0) misparses ?[...] inside ternary expressions.
    final reciter = ref.watch(selectedReciterProvider);
    final qdcId = reciter?.qdcReciterId;
    final appId = reciter?.id;

    final qdcTimingsMap = qdcId != null
        ? ref.watch(surahWordTimingsProvider('$qdcId:${surah.id}')).valueOrNull
        : null;
    final localTimingsMap = (qdcId == null && appId != null)
        ? ref.watch(localSurahWordTimingsProvider('$appId:${surah.id}')).valueOrNull
        : null;

    final qdcTimings = qdcTimingsMap?[ayah.ayahNumber];
    final localTimings = localTimingsMap?[ayah.ayahNumber];
    final timings = qdcTimings ?? localTimings;

    if (!isAudioActive) {
      return _buildWordWrap(context, words, -1);
    }

    // Active ayah: compute highlighted word from live position.
    // audio.duration is per-verse for ConcatenatingAudioSource (just_audio resets
    // both position and duration to the current item on each track change).
