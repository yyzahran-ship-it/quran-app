...
import { getLabel, SafeAreaProviderCompat, Screen as ScreenContent } from '@react-navigation/elements';
...
function reducer(state, action) {
  switch (action.type) {
    case 'CONFIRM_STATE':
      return state.confirmed.routeKey === action.confirmed.routeKey && state.confirmed.provenance === action.confirmed.provenance ? state : {
        ...state,
...
export function NativeBottomTabView({
  ...
  const navigate = (route, confirmed) => {
    dispatch({
      type: 'CONFIRM_STATE',
      confirmed
    });
  ...
  const onTabSelected = event => {
    ...
    if (!route) {
      console.error(`Received 'tabSelected' for route that doesn't exist: ${selectedScreenKey}`);
      return;
    }
  ...
  const onTabSelectionRejected = event => {
    const {
      selectedScreenKey,
      provenance
    } = event.nativeEvent;
    ...
    const route = state.routes.find(route => route.key === selectedScreenKey);
    if (!route) {
      console.error(`Received 'tabSelectionRejected' for route that doesn't exist: ${selectedScreenKey}`);
      return;
    }
  ...
  const onTabSelectionPrevented = event => {
    const {
      selectedScreenKey,
      provenance,
      preventedScreenKey
  ...
  return /*#__PURE__*/_jsx(SafeAreaProviderCompat, {
    children: /*#__PURE__*/_jsx(Tabs.Host, {
      ...
      children: state.routes.map((route, index) => {
        ...
        const normalTabItemAppearance = {
          tabBarItemTitleFontColor: inactiveTintColor ?? fontColor,
          tabBarItemIconColor: inactiveTintColor
        };
        const selectedTabItemAppearance = {
        ...
        return /*#__PURE__*/_jsx(Tabs.Screen, {
          ...
          children: /*#__PURE__*/_jsx(Deferred, {
            lazy: lazy,
